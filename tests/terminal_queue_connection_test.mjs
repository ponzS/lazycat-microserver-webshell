...
test("unified connection multiplexes three live panes over one physical socket", async () => {
  ...
  physical.emit("message", {
    data: JSON.stringify({
      ...
      agent_protocol_version: "lcmd-webshell-agent-v9",
  ...
  assert.equal(physicalEvents.at(-1).agentProtocolVersion, "lcmd-webshell-agent-v9");
  assert.equal(physicalEvents.at(-1).preferredAgentProtocolVersion, "lcmd-webshell-agent-v10");
  assert.equal(physicalEvents.at(-1).agentProtocolUpdateAvailable, true);
  assert.equal(physicalEvents.at(-1).agentProtocolUpdateRequired, false);
  assert.equal(connection.snapshot().agentProtocolUpdateAvailable, true);
  assert.equal(connection.snapshot().agentProtocolUpdateRequired, false);
