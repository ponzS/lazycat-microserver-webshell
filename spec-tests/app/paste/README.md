...
规格：[REQ](../../../spec/app/paste/REQ.md) · [AC](../../../spec/app/paste/AC.md)。环境与目标：[ENVIRONMENT](../../ENVIRONMENT.md)。
单模块：`./run-ac.sh --selector app/paste`（产品根目录）。
...
1. 把普通文本通过系统粘贴输入终端。
2. 把剪贴板中的图片或文件粘贴到终端。
3. 在桌面端通过附件文件选择器手动上传，等待远端路径复制成功后，不额外点击终端便立即执行系统粘贴。移动端文件选择器成功后自动写入路径由 `app/mobile-upload-path` 验收。
...
node spec-tests/run-playwright.mjs spec-tests/app/paste/test.mjs
```
...
- `spec-tests/run-playwright.mjs` 在页面创建前仅为测试目标 origin 授予 Chrome 150+ 的 `local-network-access` 权限，恢复真实 Provider WebSocket，不关闭浏览器安全特性。
...
- 完整 `./spec-tests/test-all.sh` 结果以本轮最终验证记录为准。
