...
controller 唯一持有 metrics retry 的 RAF/timer 集合，并以 session generation 拒绝迟到回调；session cleanup 会取消该 session 的 timer。字体变化先调用 presentation hold，再更新 live pane options、renderer metrics 并请求 resize；因此字体变化期间的画质由 rendering 的 hold canvas backing DPR 和 presentation commit 时序共同决定，不能只从字体 metrics 或 WebSocket 日志判断。scrollback 变化通过显式 history callback 只通知 `client:` IndexedDB 兼容 owner，普通容器不创建浏览器历史任务。`dispose()` 取消所有资源，之后不再修改 session。
