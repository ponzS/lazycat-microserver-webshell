...
模块外部只能通过各目录的 `index.js` 使用公开 API。不得从 `main.js` 或其他模块深度导入内部实现，不得复制全局状态，也不得显示 history replay、snapshot、原子 resize 或重连中间过程。桌面分屏/窗口的 live geometry 只允许呈现当前 session 的真实 Canvas，不改变 replay 与恢复的原子提交边界。
