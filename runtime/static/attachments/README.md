...
本目录负责附件弹层、主动剪贴板导入、文件选择上传、原生粘贴文件上传、上传进度面板、远端文件浏览、排序、选择和下载。上传仍限制为单批最多 32 个文件、单文件最大 2GB；下载最多 64 个条目。文件浏览器会按当前目标和项目 Git root（无法识别时使用首次目录的父目录）持久化上次目录及排序方式，并在打开/导航/刷新后将列表滚动到顶部。
