...
v35 显式兼容 v34/v33/v32/v31/v30/v29/v28/v27 及原容器兼容版本，不能因此次升级自动杀掉兼容容器 agent。本次只增加客户端的可选按需指标，容器入口和 v30 的 Local `queue-ready` 握手不变。二进制名、`agent` 子命令及 LPK 内的 `runtime/` 布局保持不变；Provider 不编入独立 SSH module。
