...
v33 显式兼容 v32/v31/v30/v29/v28/v27 及原容器兼容版本，不能因此次升级自动杀掉兼容容器 agent。本次只调整客户端 SSH 密码及认证等待，容器入口和 v30 的 Local `queue-ready` 握手不变。二进制名、`agent` 子命令及 LPK 内的 `runtime/` 布局保持不变；Provider 不编入独立 SSH module。

版本是否最新、是否可 attach、是否支持工作区恢复、是否可导入内存快照是不同判断：v27/v28/v29/v30/v31/v32 继续提供标签排序和工作区恢复，v26/v27/v28/v29/v30/v31/v32/v33 使用同一快照 WASM；更早的兼容 agent 沿用字节回放。以后升级不能只修改版本常量而漏掉这些能力边界。
