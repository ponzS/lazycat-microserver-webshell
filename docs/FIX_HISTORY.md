...
- 页面不注册 Service Worker，不提供 Web App Manifest/PWA 图标，也不申请浏览器持久存储。`index.html` 在版本化资源之前仅对已有 controller 的页面触发现有 registration 更新；bootstrap 在新版代码成功启动后执行一次性旧 WebShell Worker 和已知 Cache 名称清理；Provider 在历史 `/service-worker.js` URL 提供无 fetch/预缓存/claim 的退役 Worker，用于旧受控页面删除已知缓存、注销 registration 并一次性重载。干净用户不请求 Worker 或增加导航；三条迁移路径都不读取终端数据、不参与首屏、离线 fallback 或资源调度。
...
- tab 总览优先复制已完成提交的 live Canvas，其次使用 identity 仍有效的 `terminal-frame-hold`，最后可读取 `terminal/overview/` 独立 IndexedDB 中同 selector/workspace/tab/pane identity 的派生图片 Blob。持久图片只能由 replay committed 且 presentation current 的 Canvas 生成，不含 PTY 字节/cursor/Ghostty 状态；总览不得触发 replay、恢复终端或参与输入就绪判断。
...
### LCMD-20260831-49：已打开 Unified 连接中新建终端先发送 priority 导致 logical stream 被拒绝

- 日期：2026-08-31
- 来源：移除 PWA/Cache API 后的现场回归；已有终端正常呈现时点击新建终端可稳定复现，刷新浏览器后恢复正常。
- 错误现象：初始 pane 正常，但在同一页面新建 tab/终端后，新 pane 持续黑屏并显示连接红点。日志反复出现 `unified pane stream is not active`，logical stream 按可重试错误循环关闭、重建，最终进入 replay pause；刷新后所有 pane 又能正常显示。
- 根因：物理 Unified WebSocket 首次打开前，完整 membership 会一起随 `replace-subscriptions` 发布，所以刷新路径正常；物理连接已经打开后新增 pane 时，`setPriority()` 会立即发送新 identity 的 `set-priority`，而 `replace-subscriptions` 仍在后续 microtask 中。Provider 尚未注册该 pane/stream/channel generation，严格身份校验因此正确拒绝控制帧。前端把拒绝当成 logical stream 故障并重试，形成稳定黑屏。错误顺序为 `set-priority(new pane) -> replace-subscriptions(new pane)`，正确顺序必须是 `replace-subscriptions -> set-priority -> pane-control`。
- 实施方案：在 `terminal_queue_connection.js` 集中规范化 priority 并复用 `sendPriority()`；新 logical stream 的 `lastSentPriority` 仍为 `null`，或 subscription 更新已经 pending 时，`setPriority()` 只更新本地 subscription 并等待 `sendSubscriptions()`。只有 `replace-subscriptions` 成功发送后才提交该 membership 的 `lastSentPriority`，并为已经注册且 priority 确实变化的旧 stream 补发 `set-priority`。Provider `terminal_queue.go` 的严格 pane/stream/channel generation 校验保持不变，不通过放宽身份检查掩盖前端时序错误。
- 呈现边界：修复只调整 Unified 控制帧发布顺序，不改变 PTY、history、snapshot、resize 或 Canvas 状态所有权；新 pane 仍在 replay suppression 中完整消费服务端权威 snapshot/live，replay complete、队列追平、fit 和最终 full render 完成前不得显示任何中间帧。
- 回归 guard：`terminal_queue_connection_test.mjs` 新增“opened physical transport publishes new logical subscription before priority updates”，固定已打开物理连接后新增 pane、连续 priority 变化时发送队列先为空，microtask 后第一帧必须是包含最终 priority 的 `replace-subscriptions`，随后才允许旧 stream 的必要 `set-priority`。`tests-auto/06-workspace-tabs/test.mjs` 在初始终端就绪后点击真实 `#newTab`，等待新 pane connection/render/presented 全部 ready，检查 Canvas 非空、无 `unified pane stream is not active`，并确认前后始终只有 1 条已创建且活动的 Unified WebSocket。
- 验证结果：定向 Node 测试 `29/29`、Node 全量 `364/364`、`go test ./... -count=1`、`go test -race ./... -count=1`、全部生产 JavaScript `node --check` 和 `git diff --check` 通过。真实浏览器通过 `xvfb-run -a ./tests-auto/run-visible.sh tests-auto/06-workspace-tabs` 使用当前工作区静态资源完成，artifact 为 `tests-auto/06-workspace-tabs/artifacts/2026-08-31T06-42-46-242Z`；新终端 Canvas 为 `1440x861`、非透明像素 `20326`，新建前后 socket 均为 `created=1, active=1`。首次直接运行 visible 脚本因当前 shell 无 `DISPLAY` 导致 Chrome 在用例前退出，不属于代码失败。用户随后在真实设备手动验证通过。
- 禁止复现：不得在 Provider 接收并应用 `replace-subscriptions` 前发送新 identity 的 priority/control；不得通过忽略 `unified pane stream is not active`、放宽 Provider identity 校验或额外建立物理 WebSocket 规避问题；单 pane 失败不得关闭兄弟 stream，任何重试、snapshot、replay 或 resize 过程都不得显示中间帧。

### LCMD-20260831-50：移除 Service Worker 路由后旧 registration 无法退役并导致升级白屏

- 日期：2026-08-31
- 来源：PC 客户端从仍注册 PWA Worker 的旧 LPK 直接升级到 `LCMD-20260831-48` 后现场反馈；其他没有旧 registration 的设备访问正常。
- 错误现象：受影响客户端更新后仍保留并加载旧 `service-worker.js`，其 app-shell 索引对应的旧资源已经不存在或为空，页面持续白屏；刷新和重新进入仍无法恢复。新设备或清除站点数据后正常。
- 根因：PWA 移除时同时删除了 Provider 的 `/service-worker.js` 路由。浏览器对旧 registration 执行更新检查时收到 404，无法安装替代 Worker，因此继续保留旧 active Worker。新版页面内的 `legacy_storage_cleanup_controller.js` 只有在版本化 `main.js` 和 `global-runtime.js` 已成功执行后才会注销 registration；一旦旧 Worker 已让页面在资源加载阶段白屏，这条清理链永远没有执行机会，形成升级死锁。
- 实施方案：Provider 在原 `/service-worker.js` URL 恢复专用退役响应，以 `Cache-Control: no-store`、JavaScript content type 和原 root scope 返回 `app/bootstrap/legacy_service_worker_retirement.js`。退役 Worker 不包含 fetch listener、预缓存或 `clients.claim()`；install 立即 `skipWaiting()`，activate 先捕获当前受控 window clients，精确删除 `lcmd-webshell-app-shell-*` 与 `lcmd-webshell-terminal-v2`，再注销自身并导航这些受控窗口一次。当前页面仍不注册 Worker；成功启动后的既有清理 controller 继续作为第二层 fallback。
- 安全边界：干净用户没有 registration，不会请求、安装或执行退役 Worker，因此不会增加首次进入重载。只有旧 registration 控制的窗口会在 Worker 更新后自动导航一次；默认 `clients.matchAll({ type: "window" })` 不包含未受控页面。退役脚本不使用 `Clear-Site-Data`，不删除 localStorage、设置或 `client:` IndexedDB，也不恢复 PWA、离线或浏览器终端缓存。PTY 和工作区权威状态仍位于 persistent agent，页面恢复只重新 attach。
- 回归 guard：`legacy_service_worker_retirement_test.mjs` 在隔离 Worker 上下文执行 install/activate，覆盖 `skipWaiting`、受控窗口筛选、已知 Cache 精确删除、单个删除/导航失败不阻塞、先注销后导航，并禁止 fetch/cache open/client claim；`TestLegacyServiceWorkerRetirementHandler` 固定 GET/HEAD/405、`no-store`、JavaScript 类型和 root scope；`TestRuntimeSnapshotOnlyAndPWARemovalContract` 固定旧 URL 路由存在但页面不得重新注册 Worker，退役脚本不得恢复 PWA 行为。
- 验证结果：退役 Worker 与旧页面清理器定向 Node 测试 `4/4`，Node 全量 `366/366`、`go test ./... -count=1`、`go test -race ./... -count=1`、全部生产 JavaScript `node --check` 和 `git diff --check` 通过。`lzc-cli project release` 成功生成 `cloud.lazycat.webshell.lcmd-v1.0.39.lpk`，SHA-256 为 `4b9a085f38a5332802e1853cbb5a52d001c3ef2f8cc3494f42693fa4e91f1880`，content revision 为 `fed211ab5e9d2a04a97da7d0b5ccce4f4a79a1434b1e0919d2d93e9ff17d6192`；包内退役 Worker SHA-256 与工作区一致为 `63fc14725dac52ed615ecc1c5856d276480c4e02a78258153e49399ff60cdcb1`，且不包含旧 app-shell `service-worker.js`、Manifest、PWA 图标或 Cache v2 文件。真实 Chrome 升级用例 `tests-auto/11-service-worker-retirement/artifacts/2026-08-31T07-24-56-486Z` 通过：旧页面 `controlled=true` 且两个旧 Cache 存在，更新后自动导航恰好 1 次、`controlled=false`、registration/cache 全空；干净移动浏览器没有 registration，访问 fixture 后 Worker 请求数仍保持 2，证明新用户不会触发退役脚本；随后桌面和移动页面均返回真实 `debug123` WebShell 并恢复终端。现有 `tests-auto` 真机矩阵 01-03、05-10 全部通过；02 首次大输入等待超时后单独复验通过，04 首次因未设置 iPhone UA 未进入目标分支，补充正确 UA 后复验通过。尝试把当前 LPK 直接安装到默认 `debug123` 时，设备侧提示“懒猫开发者工具尚未安装或无法通过公开路径访问”，因此尚未完成不经过测试 fixture 的旧 LPK -> 当前 LPK 原位升级；Provider 路由/响应头由 Go 测试覆盖，浏览器 Worker 生命周期使用当前工作区退役脚本完成。
- 禁止复现：移除页面 Service Worker 注册时不得直接让历史 script URL 永久返回 404 或空脚本；退役 Worker 必须保持无 fetch/预缓存/claim，并在注销后只导航原受控窗口一次。不得用 `Clear-Site-Data`、全站缓存清理、每次进入强制 reload 或重新启用 PWA 绕过升级问题。

### LCMD-20260831-51：旧 PC WebView 未及时检查退役 Worker 更新并继续运行旧 app-shell

- 日期：2026-08-31
- 来源：部署 `LCMD-20260831-50` 后 PC 客户端手动回归失败；服务器直接请求 `/service-worker.js` 已返回 1126 字节的新退役脚本，但 DevTools 中 active Worker 仍是包含 `content-b4bcf13cae603eb69721020f` app-shell 清单的旧实现，其他无历史 registration 的设备正常。
- 错误现象：PC 客户端升级后仍由旧 active Worker 控制，继续使用已经失效的旧资源索引并白屏；服务器路由和退役脚本内容已经正确，单纯再次刷新不保证 WebView 立即安装新 Worker。
- 根因：第一版只恢复了历史 script URL，依赖浏览器自行安排 Service Worker 更新检查。目标 PC WebView 没有在这次页面进入时及时执行检查，因此退役脚本从未进入 install/activate。真实浏览器回归又由测试代码直接调用 `registration.update()`，验证了退役 Worker 的后半段，却没有验证生产页面是否能主动启动更新，形成测试盲区。
- 实施方案：在 `index.html` 的任何版本化脚本和样式之前加入最小内联触发器。它先检查 `navigator.serviceWorker.controller`；只有当前页面仍被旧 Worker 控制时，才通过 `getRegistration("./")` 获取同 scope registration 并调用一次 `update()`。触发器不注册 Worker、不访问 Cache、不直接 reload；更新成功后的缓存删除、unregister 和单次导航仍由 `legacy_service_worker_retirement.js` 唯一负责。
- 安全边界：干净用户没有 controller，触发器同步返回，不请求 `/service-worker.js`、不等待异步流程，也不增加页面导航。旧用户进入页面时只多一次明确的 Worker 脚本更新请求；退役 Worker 激活后只导航原受控窗口一次。更新失败被局部吞掉以允许页面继续加载，成功启动新版 JavaScript 后仍有 `legacy_storage_cleanup_controller.js` 作为 fallback。
- 回归 guard：`TestHandleIndexForcesLegacyWorkerUpdateBeforeAssets` 和 `TestRuntimeSnapshotOnlyAndPWARemovalContract` 固定触发器必须位于版本化资源之前、仅对已有 controller 调用 update，并禁止注册 Worker、直接 reload 或打开 Cache。`tests-auto/11-service-worker-retirement/test.mjs` 必须从生产 `index.html` 提取真实触发脚本，通过一次普通导航完成升级，禁止测试代码直接调用 registration update；断言该用户导航之外只有一次退役导航、旧 registration/Cache 全部清除，并断言干净浏览器不请求 Worker 且没有额外导航。
- 验证结果：退役 Worker/旧存储清理定向 Node 测试 `4/4`、Node 全量 `366/366`、定向 Go、`go test ./... -count=1`、`go test -race ./... -count=1`、全部生产 JavaScript `node --check` 和 `git diff --check` 通过。真实 Chrome 用生产 `index.html` 前置脚本完成升级，artifact 为 `tests-auto/11-service-worker-retirement/artifacts/2026-08-31T07-55-12-637Z`：一次用户导航后恰有一次退役导航，document 请求从 1 增至 3、Worker 请求从 1 增至 2，最终 `controlled=false`、registration 和旧 Cache 全空；干净移动浏览器访问后 document 只增加 1、Worker 请求仍为 2，证明无额外导航或 Worker 请求。当前工作区静态资源连接真实 `debug123` API/WebSocket/PTY 的 `tests-auto` 01-10 矩阵全部通过，04 使用 iPhone UA 进入 iOS 分支；01 首次尺寸所有权切换瞬时不一致、02 前两次分别在普通回显和 20KB 大输入回显超时，单独复验均通过，其余用例一次通过。`lzc-cli project release` 成功生成 `cloud.lazycat.webshell.lcmd-v1.0.39.lpk`，SHA-256 为 `8ddf11606a881e13aab2cc1d16f35f83cf795545bcd2e4e36300a7fae7c30a6b`，content revision 为 `4671fddd80db96b3a4d9277231827fa7734ac59b05bba314efa65b5528a5d385`；包内 `index.html` 与退役 Worker SHA-256 分别为 `8e6b1328ca8a6c54255680fbbf76502e8fe3b5a01a59dcfd2ed0ed65d80cbf23`、`63fc14725dac52ed615ecc1c5856d276480c4e02a78258153e49399ff60cdcb1`，与工作区一致，且不包含旧 `service-worker.js`、Manifest 或 PWA 图标。用户随后在原先稳定复现白屏的 PC 客户端完成当前 LPK 原位升级手动测试，旧 Worker 成功退役，页面正常进入且未再出现永久白屏，验证通过。
- 禁止复现：不得再次依赖 WebView 的隐式更新时机；不得把 `registration.update()` 移到版本化应用代码之后；不得对无 controller 的新用户请求 Worker 或强制 reload；真实浏览器用例不得用测试代码手动更新 registration 掩盖生产触发链。

### LCMD-20260831-52：折叠屏开合与跨设备尺寸焦点未触发显式 claim 导致黑屏和分辨率滞后

- 日期：2026-08-31
- 来源：折叠屏手机现场回归；终端页面开合后容易黑屏，未黑屏时 PTY 仍保持旧屏幕或其他设备的 cols/rows，必须点击终端后才恢复。
- 错误现象：PC 与手机同时 attach 同一共享 PTY 时，PC 持有 resize owner；手机折叠/展开、旋转、切换 tab/pane 或从后台恢复后，页面可能继续使用 PC/旧折叠态尺寸。折叠期间多条 window、VisualViewport、orientation 和 presentation 调度交错时，还可能进入 `renderReady=false` 且最终尺寸事务未收口的黑屏状态；点击终端后 IME pointer/touch claim 生效，画面和尺寸立即恢复。
- 根因：`resizeActiveTabForCurrentDevice()` 只执行普通 fit，没有携带 `claim: true`；Provider 对非 owner 的普通 resize 正确返回 `resize_owner_active`。focus/visible/pageshow 又先发送普通 resize、再调用 claim，同 target 的在途请求会把后续 claim 以 `pending_target_same` 去重。tab/pane 激活只调度普通 resize；viewport 仅把 portrait/landscape 变化视为方向变化，无法识别 portrait-to-portrait 折叠。全局 app resize 与 viewport listener 还分别触发终端 resize，缺少统一 viewport generation。首次真实浏览器回归进一步确认：即使 viewport 已准备最终 claim，presentation 也会在两帧稳定检查完成前因 Canvas geometry 不匹配抢先调度 `claim:false`，导致服务端先应用中间尺寸，再补发最终 claim。
- 实施方案：resize controller 新增 `claimTabForCurrentDevice()`、`claimActiveTabForCurrentDevice()`、只读 `isCurrentDeviceClaimRequired()` 和 owner-aware `schedulePresentationResize()`，并持有 `pendingSizeClaim*`。显式 claim 遇到普通 resize 在途时记录 latest-only 升级意图，ACK 或 `resize_owner_active` 后重新测量最终 DOM geometry 并发送新 claim；owner 拒绝期间清理旧 fence/suppression，但保留 last-known-good hold，不应用远端过渡尺寸。viewport model 增加 layout/visual viewport、screen、DPR 和 orientation signature；viewport controller 用两帧稳定检查和 180ms 有界 fallback 合并 window/VisualViewport/orientation 事件，软键盘期间延迟 claim，收起后只提交最终尺寸，并公开只读 `isGeometryClaimPending()`。presentation 在任一 claim 门禁期间只保留 last-known-good frame、继续 validation/retry，不调度被动 geometry resize，避免远端 owner observation 后把本机 host fit 当成使用意图反抢；本设备已 claim 后若 ACK 应用新网格才暴露最终字体/行高度量，后续 presentation geometry 修正继续携带 `claim:true`，不得降级为被动帧。用户 tab/pane 切换和 pointer、focus、visible、pageshow 统一调用显式 claim；权威 workspace state apply 明确禁用 claim。`global-runtime.js` 删除第二条终端 window-resize 调度，只保留 viewport owner 的命令接线。
- 回归 guard：`terminal_resize_controller_test.mjs` 覆盖普通 resize 在途时 claim 升级、owner 拒绝后旧网格/hold 保留并重发 claim、稳定 claim 零重复帧；`terminal_viewport_controller_test.mjs` 覆盖 portrait-to-portrait 分阶段折叠只提交最终 generation、动态 pending 门禁、键盘高度变化不 claim、orientation/dispose fence；`terminal_presentation_controller_test.mjs` 固定 pending 期间零被动 resize；tab、pane 和 runtime recovery Node 测试固定用户切换使用 claim、state apply/稳定重复激活不抢 owner。`runtime_shortcuts_test.go` 固定 viewport/resize/presentation owner 与单一恢复事务。`tests-auto/01-multi-device-resize-sync` 增加 PC owner 下手机无需点击的同方向折叠/展开接管、控制帧和 resize trace 诊断，`tests-auto/04-terminal-viewport` 增加 claim 帧和原子保帧检查。
- 验证结果：定向 resize/viewport/presentation/workspace/recovery Node 测试通过，Node 全量 `375/375`、`go test ./... -count=1`、`go test -race ./... -count=1`、全部生产 JavaScript `node --check` 和 `git diff --check` 通过。真实 `debug123` Provider/persistent agent/PTY 回归使用当前工作区 `runtime/static` 映射：严格 `01-multi-device-resize-sync` 连续 3 次通过，完整矩阵中再通过 1 次；展开和折回阶段均只有最终 `claim:true` 控制帧，PC/手机交替接管、PTY 输出同步与 Canvas 非空通过。`04-terminal-viewport`、`08-terminal-click-jitter`、`09-terminal-interaction-jitter`、`10-terminal-geometry-jitter` 通过，完整 01-11 矩阵全部用例最终通过。矩阵首次运行到 09 时测试机同时出现 HTTP 502、PTY EOF 和 WebSocket 握手返回 200，marker 等待超时；artifact 为 `tests-auto/09-terminal-interaction-jitter/artifacts/2026-08-31T09-43-11-958Z`，服务恢复后同一用例原样复跑通过，artifact 为 `tests-auto/09-terminal-interaction-jitter/artifacts/2026-08-31T09-45-05-354Z`。`lzc-cli project release` 成功生成 `cloud.lazycat.webshell.lcmd-v1.0.39.lpk`，包内 `.lpk-version` 为 `1.0.39`，包含本次 resize/viewport/presentation 实现，SHA-256 为 `0bcf648bd459bd9ed050c108e936b853150dbb21bd694cbe0dfef153f3a7c8dd`。折叠屏真机开合、实体软键盘/屏幕组合和实际 PC/手机切换仍需用户手动验收。
- 禁止复现：不得把所有 ResizeObserver/字体/主题等被动变化改成无条件 claim；不得先发普通 resize 再补 claim；不得按相同 pending target 丢弃更强的 claim 意图；不得只用 portrait/landscape 判断折叠变化；不得让 app lifecycle 与 viewport 分别维护终端 resize 调度；任何折叠、claim、ACK/error、snapshot、replay 或重连过程都不得显示中间帧，最终 claim ACK 前必须保留 last-known-good frame。

### LCMD-20260831-53：折叠/全屏 resize ACK 边界污染与总览预览跨重载丢失

- 日期：2026-08-31
- 来源：继续处理折叠屏开合回归，并结合用户新增现场：切 tab、F11 进出全屏或快速改变终端尺寸时，当前会话偶发出现其他会话历史、旧行或乱码；退出重进后污染消失。同时发现应用退出重进后，终端总览只有当前会话有预览，其余会话显示“无预览”。
- 错误现象：resize 请求发送后到 `resize-applied` ACK 返回前，PTY 二进制输出仍可能抵达。部分情况下这些字节按新 Ghostty cols/rows 解析，短暂污染当前 Canvas；旧页面重建历史后污染自然消失。移除 Cache API preview 后，后台 pane 的已呈现画面只存在于旧页面内存，应用重载时总览没有可用 live/hold Canvas，只能显示空缩略图。
- 根因：resize fence 在发送请求时记录了 output queue entry 数量；ACK 到达前新入队的旧尺寸字节没有计入 drain 边界，ACK 后 local `term.resize()` 可能早于这些字节排空。总览原有 live/hold 来源没有跨页面持久化能力，且不能重新读取 PTY 历史补图；这不是 Unified pane 路由串流证据，而是本地几何事务和派生展示状态的两个边界缺口。
- 实施方案：matching `resize-applied` ACK 处理器重新读取完整 `getOutputQueueEntryCount(session)` 并重置 `resizeFenceDrainRemainingEntries`，在全部 ACK 前 queue 按旧网格排空后才应用 `term.resize()`；ACK handler 返回后到达的字节留给新网格。该 fence 继续使用 output controller 的公开 bounded flush，不触碰 history/replay 或 Unified 物理连接。总览新增独立 `terminal/overview/preview_controller.js` 与 `preview_store.js`：只在 replay committed、presentation current 的已提交 live Canvas 上以 320ms 有界 latest-only 频率编码缩略图，保存到 `lcmd-webshell-overview-previews-v1` IndexedDB；总览来源顺序固定为 live > 有效 hold > 同 selector/workspace/tab/pane identity 的持久图片。图片记录最多 64 项、30 天过期，不含 PTY 字节、cursor、checkpoint、Ghostty buffer 或 Cache API 状态。冷启动 pane 在 history generation 尚未知时按精确 workspace/tab/pane identity 读取；generation 已知后严格拒绝旧图。pane 删除显式删除记录，应用 dispose 不清空其他有效记录；迟到 encode/decode、旧 generation 清理和 dispose 回调均不得覆盖新图或更新 UI。
- 回归 guard：`terminal_resize_controller_test.mjs` 固定 matching ACK 前所有 output entry 先按旧网格 drain；`terminal_overview_preview_test.mjs` 覆盖 IndexedDB 容量/过期/identity、跨 controller 重建、history generation 隔离、持续输出下 latest-only capture、迟到 decode/capture 和 dispose；`terminal_overview_controller_test.mjs` 固定 live/hold/persisted 来源优先级。`runtime_shortcuts_test.go` 固定 overview preview store/controller 的公开入口、global runtime capture/delete/pagehide 接线、不得持有 PTY/history/transport 状态；新增 `tests-auto/12-overview-preview-persistence`，要求真实刷新后后台 tab 的 Canvas 缩略图来自持久图片且不触发 replay 或额外 Unified socket。viewport 原有折叠几何 recovery guard 和 `tests-auto/01`、`04` 继续覆盖无点击最终 claim、频繁开合和保帧。
- 验证结果：Node 全量 `384/384`；终端 resize/viewport/presentation/transport/overview 定向 `67/67`；`go test ./... -count=1`、`go test -race ./... -count=1`、全部生产与 tests-auto JavaScript `node --check`、`git diff --check` 通过。使用当前工作区 `runtime/static` 映射真实 `debug123` Provider/persistent agent/PTY、iPhone UA 和无头 Chrome 执行 `tests-auto` 01-12 全部通过：折叠/展开无需点击完成最终 claim，resize/交互无 unsafe、旧行或乱码，后台 tab 重载后持久预览绘制成功，Service Worker 退役用例保持通过。`lzc-cli project release` 成功生成 `cloud.lazycat.webshell.lcmd-v1.0.39.lpk`，SHA-256 为 `e38224aee5a4a8c30397ece4834f9d06e1de63826a510bbda5137c68aa553fab`；包内 `runtime/static/terminal/overview/preview_controller.js`、`preview_store.js`、`overview_controller.js`、`resize_controller.js`、`viewport_controller.js` 和 `global-runtime.js` 与工作区逐文件一致，且不存在旧 Service Worker/Manifest/Cache API app-shell 资源。
- 禁止复现：不得在 resize ACK 前按新网格解析旧 queue entry；不得用 replay/reset、清空终端、重建 WebSocket 或放宽 pane identity 掩盖乱码/串会话；不得让持久 preview 读取或恢复 PTY/history/cursor，不得让旧 history generation 的迟到删除覆盖新图片；不得把 overview preview 持久化重新并入 Cache API/PWA 或让其参与 input ready、replay、resize、连接恢复；所有折叠、全屏、resize、snapshot、replay 和重连中间过程继续禁止显示。

### LCMD-20260831-54：折叠恢复等待过长、输入 resize 抖动与黑屏指示状态不一致

- 日期：2026-08-31
- 来源：折叠屏连续开合现场反馈；终端在灰点消失后仍黑屏，数秒后短暂红点再恢复；输入文字期间画面上下抖动。用户明确要求回放完成后必须稳定收敛，黑屏只显示灰点，红点仅代表网络异常或断网。
- 根因：Unified 物理通道恢复等待真实 WebSocket close 事件，异常 CLOSING 状态会把 replacement transport 阻塞在约 4 秒的 health fence；旧 logical stream 没有在物理断开时尽快释放。远端 resize epoch 通过 `applyObservedResize()` 直接改写本地 Ghostty cols/rows，可能让旧输出按新网格解析并污染画面。presentation hold 捕获失败后仍可能继续把 live Canvas 设为不可见。输入 controller 与 IME 在每个字符的 beforeinput/input/keydown 上强制 reassert size，输出恰好在 validation/settle 期间时会重新进入 hold。连接状态把普通 replay/连接重试与网络断开共用红色语义，黑屏时无法区分“正在稳定渲染”和“网络故障”。
- 实施方案：Unified controller 引入 500ms 有界 close fence，物理断开时先关闭当前 target 的 active logical sockets，再执行单次 membership/reconnect；旧 generation 的迟到 close 仍由 `closedConnections` 拒绝。远端 epoch/error 只更新 server geometry 与 `sizeClaimRequired`，保留本地 last-known-good 网格和画面，不应用远端过渡尺寸；resize 在需要 hold 但无法捕获有效 Canvas 时延后事务并保留当前 live frame。输入稳定且已有 presented frame 时取消逐字符强制 reassert，仅在 render 未就绪、claim 待接管、resize ACK 或 activation fit pending 时恢复尺寸校验。指示灯样式改为 `data-render-ready=false`、connecting/reconnecting/普通 replay error 使用灰色，`offline`/`network-error` 才使用红色；network error 路径显式写入 `network-error`，不再把普通 logical/replay 身份错误误标为红色。
- 回归 guard：`terminal_unified_transport_controller_test.mjs` 覆盖短 close fence、物理断开释放 logical stream 和 replacement recovery；`terminal_resize_controller_test.mjs` 固定远端 epoch 不应用中间本地网格、保留 presentation hold；`terminal_presentation_controller_test.mjs` 固定 hold 捕获失败时已有 live frame 仍可见；`terminal_input_controller_test.mjs` 与 IME 行为固定稳定输入不触发逐字符 resize。全量 Node、生产 JS 语法和 Go guard 需在本条收口执行。
- 验证结果：定向 Node 测试通过；全量 Node `387/387` 通过。真实折叠屏连续开合、实体输入与跨设备接管仍需用户手动测试，重点观察 2 秒内是否保持 last-known-good、是否只在明确网络断开时出现红点，以及回放完成后是否稳定渲染。
- 禁止复现：不得删除 replacement 前的短 close fence 或创建双物理连接；不得把远端观察尺寸直接应用到本地终端；不得在 hold 捕获失败时隐藏唯一 live frame；不得让普通输入、回放完成或 validation 重新触发无条件 resize/hold；不得用红点表示普通 connecting、replay、resize 或渲染等待状态，任何 replay、snapshot、resize、重连中间帧都不得显示。

### LCMD-20260831-55：物理断开后的 logical 重试覆盖网络错误指示

- 日期：2026-08-31
- 来源：折叠屏快速开合/网络抖动时观察到红点只短暂出现，随后 logical stream 重试又恢复为灰点，而终端仍处于黑屏等待状态。
- 根因：Unified 物理 owner 在检测到断开时先把活动 pane 标记为 `network-error`，随后 pane WebSocket 的 close 回调和 `scheduleUnifiedPaneRetry()` 无条件写入 `reconnecting`，丢失了物理断开的原因。
- 实施方案：协议 close 处理按物理通道丢失、网络/WebSocket/传输异常原因选择 `network-error`，普通 logical 重试和 replay/尺寸等待保持 `reconnecting` 灰点；Unified 物理 owner 将实际断开原因保留到 pane 的 logical close 回调；WebSocket `error` 直接进入网络错误状态。该状态只影响指示灯和诊断，不改变连接恢复、PTY 回放或渲染调度。
- 回归 guard：`terminal_session_protocol_controller_test.mjs` 固定物理 Unified 断开经过 logical close 后仍保持 `network-error`；`terminal_unified_transport_controller_test.mjs` 继续固定物理 owner 为活动 pane 标记网络错误且只触发一次恢复；样式 guard 固定 `renderReady=false`/connecting/reconnecting 为灰色、`offline`/`network-error` 为红色。
- 验证结果：新增协议状态测试、Node 全量 `388/388`、`go test ./... -count=1`、`go test -race ./... -count=1`、全部生产/测试 JavaScript `node --check` 和 `git diff --check` 均通过；使用当前工作区 `runtime/static` 映射并显式注入 iPhone UA 的真实 Chrome `tests-auto` 01-12 全量通过。未注入移动 UA 的首次 04 组失败仅因测试环境未进入 iOS 分支，补齐测试参数后原样复跑通过。
- 禁止复现：不得让物理网络断开在 logical 重试开始后降级为灰点；不得把普通 replay、resize、切 tab 或 logical stream 重试标记为红点。

### LCMD-20260831-56：前台恢复入口缺少统一代次和运行时竞态 trace

- 日期：2026-08-31
- 来源：终端调度简化计划阶段 1/2 的代码审计；现场表现为后台恢复慢、折叠/恢复时多个入口同时触发连接、尺寸和呈现检查，旧在线等待回调可能在新恢复开始后继续执行。
- 错误现象：`visibilitychange`、`pageshow`、`focus`、`online` 和用户手势分别调用 transport probe、session health、active tab claim、activity refresh 等流程；连续事件会重复启动恢复。问题发生时日志只能看到多个局部事件，无法判断它们是否属于同一次恢复，旧 promise/timer 是否覆盖了新状态。
- 根因：应用 recovery lifecycle 只有用于在线等待的通用 generation，没有把所有前台恢复信号绑定到一个可观测的 resume generation；重复入口之间没有短窗口合并，旧回调也没有统一的 stale 记录。诊断时间线只记录 pane 事件，缺少页面级恢复事件和来源。
- 实施方案：diagnostics 新增模块内的有限运行时事件时间线，并在写入前脱敏；终端 session 时间线自动附加当前 `resumeGeneration`，不向 session 写入诊断数组。`runtime_recovery_lifecycle.js` 新增前台恢复代次和 64ms 合并窗口，`runtime_recovery_controller.js` 对 visibility/pageshow/focus/online 只接受一个有效代次，记录 `resume_signal`、`resume_dispatch_start`、`resume_dispatch_complete`、`resume_signal_coalesced` 和 `resume_callback_stale`；显式用户手势可强制开始新代次。离线会重置合并窗口并使在线等待失效。该批不改变恢复事务内部的 geometry、transport、replay、drain、render 顺序。
- 安全边界：诊断状态继续由 diagnostics 模块的 WeakMap/私有数组持有，不修改业务 session；运行时 trace 只保留有限条目，敏感字段统一脱敏，不记录 PTY 原文、命令、token 或 cookie。诊断事件不驱动业务状态，旧 Service Worker、Cache API、PWA 和终端历史路径不变。
- 回归 guard：`diagnostics_controller_test.mjs` 覆盖运行时 trace、resume generation、敏感字段脱敏和 session 不被写入；`app_runtime_recovery_controller_test.mjs` 覆盖 visibility/focus/pageshow 合并、用户手势强制恢复、在线等待在 offline 后的 stale callback，以及原有恢复调用顺序；app/diagnostics README 和 `docs/FRONTEND_MODULE_MAP.md` 同步记录状态 owner。
- 验证结果：定向 recovery 6/6；Node 全量 `391/391`；`go test ./...`、`go test -race ./...`、生产 JavaScript `node --check` 和 `git diff --check` 通过。真实折叠屏、WebView 后台恢复和多设备场景尚未在本条执行，作为下一阶段设备验收遗留。
- 禁止复现：不得为每个生命周期事件重新创建独立恢复状态机或绕过 resume generation；不得让诊断 trace 反向驱动连接/回放/resize；不得将 coalesced、replay、resize 或 logical retry 误标为网络红点；任何历史回放中间帧仍禁止显示。

### LCMD-20260831-57：恢复 deadline 与普通 logical retry 被错误标记为网络故障

- 日期：2026-08-31
- 来源：继续执行终端调度简化计划阶段 2/7 时的代码审计和现场状态原则复核。
- 错误现象：恢复、回放、logical attach 或 resize 等待期间，部分 pane 的小点会短暂变成红色；当 `renderReady=false` 且确有网络故障时，样式顺序又可能把红点覆盖成灰色。恢复超过用户可接受时长时缺少统一的 2 秒可观测节点，无法区分“仍在渲染”与“网络失败”。
- 根因：`session_connection_controller.js` 在普通 `scheduleReconnect()` 中对固定字符串 `network_retry` 调用网络故障判定，导致所有 direct retry 都进入 `network-error`；Unified 物理 owner 对在线断开无条件写入 `network-error`，把 logical attach/replay 失败和真实物理故障混为一谈；CSS 的 `[data-render-ready="false"]` 灰色规则位于网络错误规则之后，覆盖了真实红点。
- 实施方案：普通 retry 固定写 `reconnecting`，只有携带 network/WebSocket/physical/transport 等明确原因的路径才写 `network-error`；Unified 物理 owner 按断开原因判定，在线 logical 失败保持灰点，离线仍写 `offline`。连接指示器 CSS 将显式 `offline`/`network-error` 红色规则放到灰色 render-pending 规则之后，网络故障不会因渲染等待被降级。应用恢复 controller 为每个 accepted `resumeGeneration` 注册 2 秒 deadline timer，记录 `resume_deadline_met`/`resume_deadline_exceeded` 和对应 metric；超时通过显式回调维持灰色待恢复状态，旧代次和 dispose 的 timer 由 generation fence 清理/拒绝。
- 安全边界：deadline 只提供诊断和灰色状态收口，不改变 geometry、transport、history、replay、output 或 presentation 顺序；不清空 last-known-good frame，不显示 replay 中间帧，不因 deadline 创建额外 socket。
- 回归 guard：`app_runtime_recovery_controller_test.mjs` 覆盖 deadline 超时灰色语义、deadline met、旧代次回调；`terminal_session_connection_controller_test.mjs` 固定普通 retry 为 `reconnecting`；`terminal_unified_transport_controller_test.mjs` 固定 logical unified disconnect 在线时为灰色、physical failure 仍为红色；`runtime_shortcuts_test.go` 固定网络红色规则覆盖在 render-pending 灰色规则之后。
- 验证结果：本批定向 Node 测试 `22/22` 通过；全量 Node `395/395`、`go test ./...`、`go test -race ./...`、生产 JavaScript `node --check` 和 `git diff --check` 均通过。使用当前工作区静态资源映射、真实 `debug123` Provider/persistent agent/PTY 和 iPhone UA 执行 `tests-auto` 01-12 全部通过；Android Fold UA 的 04 组仅因用例要求 iOS 分支而跳过，改用 iPhone UA 后原样复跑通过。
- 禁止复现：不得用固定 `network_retry` 或普通 replay/resize/reconnect 原因推导红点；不得让 `[data-render-ready="false"]` 覆盖真实网络红点；deadline 超时不得显示红点或触发第二套恢复状态机；任何 replay、snapshot、resize、logical retry 和 deadline 等待过程继续禁止显示中间帧。

### LCMD-20260831-58：Ghostty 渲染抑制与 viewport 副作用收口

- 日期：2026-08-31
- 来源：审计 Ghostty 定制提交 `e21dce2`、`bd6f3d1` 和 `e6730d0`，并在当前 WebShell 的真实输入回归中复现 Canvas 长时间黑屏。
- 错误现象：渲染失败后的 retry 可能被后续普通请求取消；重复调用 render suppression 的释放可能下溢；renderer 在渲染过程中反向修改 Terminal viewport；未使用的 work scheduler 增加了额外调度层。最初尝试把 suppression reason 改为计数 Map 后，现有 resize/replay 调用方的幂等 begin 被错误累计，释放次数不足，真实 02 输入用例稳定出现 `renderReady=false` 和黑屏。
- 根因：本项目的 reason API 是“每个 reason 一个活动作用域”的幂等集合，不是每次调用都必须配对的计数器；调用方会在同一 resize/replay 事务中重复 begin，但只在事务结束时 end 一次。Ghostty renderer 同时承担 viewport 归一化还会在渲染阶段触发 Terminal 状态副作用，增加重入和竞态边界。
- 实施方案：删除未使用的 `TerminalWorkScheduler` 及公开导出；`CanvasRenderer` 只执行纯 viewport clamp，`Terminal.renderNow()` 在进入 renderer 前统一调用 `normalizeViewportBounds()`；`endRenderSuppression()` 对无匹配作用域安全返回、dispose 时清零 depth；suppression 期间只保留 pending render，不创建无效 RAF，后续请求不再取消已有 retry timer。WebShell runtime 保持 reason 幂等 `Set`，同 reason 重复 begin 不增加 Ghostty depth，未知 reason end 不释放其他作用域；测试固定正常、重复和错配释放路径。
- 历史回放边界：snapshot/replay 仍始终在 suppression 下解析，只有 replay 完成、队列追平且最终 presentation full render 成功后才显示；任何中间帧不得进入 live Canvas。
- 回归 guard：`ghostty-web/lib/terminal.test.ts`、`lib/scrolling.test.ts` 覆盖 retry 保留、suppression pending、viewport clamp；`tests/terminal_runtime_controller_test.mjs` 覆盖 reason 幂等与未知释放；`runtime_shortcuts_test.go` 固定 bundle 的纯 clamp 与 Terminal 外部 normalization，并继续禁止 renderer 内调用 provider normalization。
- 验证结果：按要求重新编译 WASM，`runtime/static/ghostty-vt.wasm` 与 `ghostty-web/dist/ghostty-vt.wasm` SHA-256 均为 `65a99188312ad92780b3af2fa410b8af395536dfe1e777ec24573d8be1436f16`；Ghostty `fmt`、`typecheck`、`lint` 和 `bun test` `391/391` 通过；根目录 Node `395/395`、`go test ./... -count=1`、`go test -race ./... -count=1`、JavaScript 语法检查和 `git diff --check` 通过。真实 `debug123` 的 01 跨设备/折叠尺寸用例和 02 输入用例在最终 Set 语义下通过；Map 版本的 02 黑屏失败 artifact 保留用于回归分析，未作为最终实现依据。
- 禁止复现：不得把 reason Set 改为无调用方契约的计数器；不得让未知 end 释放当前作用域或让 Ghostty depth 下溢；不得在 renderer 内修改 Terminal viewport；不得恢复 work scheduler 或在 suppression/replay 中显示中间帧。任何 retry、resize、tab/F11、后台恢复和高频输出路径都必须最终提交稳定 full presentation。
