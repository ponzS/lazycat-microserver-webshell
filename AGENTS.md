...
- 测试以真实用户使用场景和可复现的真实故障场景为主，可以根据 spec-workflow skill 以及 spec-workflow 和 Environment MCP 的构建规范，并使用 agent-device skill、`environment/agent-device-mcp` 通用能力和 `environment/webshell-test-harness` 项目适配新增真实测试模块。
...
- `environment/` 按环境插件分类维护真实环境操作与观察服务，不属于测试模块；`environment/agent-device-mcp/` 是通用外部插件，WebShell 专属适配位于 `environment/webshell-test-harness/`。
